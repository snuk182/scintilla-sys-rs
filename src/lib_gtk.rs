use gtk::{Container, Widget, Buildable};
use glib::object::{IsA, Downcast, Object, ObjectExt};
...
use std::{mem, ptr};
use std::os::raw::{c_uint, c_int, c_void, c_ulong, c_long};
...
pub mod ffi {
	...
	impl ::std::fmt::Debug for GtkScintillaClass {
	    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
	        f.debug_struct(&format!("GtkScintillaClass @ {:?}", self as *const _))
	         .finish()
	    }
	}
	...
	impl ::std::fmt::Debug for GtkScintilla {
	    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
	        f.debug_struct(&format!("GtkScintilla @ {:?}", self as *const _))
	         .field("container", &self.container)
	         .field("_private", &self._private)
	         .finish()
	    }
	}
}
...
impl Scintilla {
    pub fn new() -> Scintilla {
    	assert_initialized_main_thread!();
        unsafe {
            Widget::from_glib_none(ffi::scintilla_object_new()).downcast_unchecked()
        }
    }
}
...
impl<O: IsA<Scintilla> + IsA<Object> + ObjectExt> ScintillaExt for O {
	fn send_message(&self, message: u32, wparam: Uptr, lparam: Ptr) -> Ptr {
		unsafe {
			ffi::scintilla_object_send_message(self.to_glib_none().0, message, wparam, lparam)
		}
	}
	fn connect_notify<F: Fn(&Self, i32, Ptr, Ptr) + 'static>(&self, f: F) -> SignalHandlerId {
		unsafe {
